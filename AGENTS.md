...
4. **Dashboard** (`/dashboard`) — Investor hub (onboarding checklist); **site header** is brand + **Dashboard** + **Sign out** when signed in, or **Login** when not. **Home** (`/`): hero CTAs **Register + Login** for guests, **Go to Dashboard** for signed-in users; **How it works** tabs section remains (no separate hero button). **Left ribbon** (`DashboardShell` + `src/lib/dashboard-nav.ts`) on all `/dashboard/*` routes — **accordion sections** (default collapsed), nav for onboarding, documents, deals, subscribe/fund, More (marketing home, hub, FAQ, profile, contact, roadmap, tech catalogue, team/sourcing); **hide / auto-hide** (localStorage). **FAQ** (`/dashboard/faq`) — accordion + TOC. **Technical catalogue** (`/dashboard/technical-catalogue`) — infra/vendor links (`src/lib/technical-catalogue.ts`). **Product roadmap** (`/dashboard/roadmap`) — Kanban + requests (`RoadmapItem`, `POST /api/roadmap`).
...
- **NextAuth (Auth.js)** — Google OAuth + credentials; JWT sessions; **`UserRole`**: `INVESTOR`, `EMPLOYEE` (email `@modularequity.com` → Employee), **`DEAL_SOURCER`** (assigned in DB — see **`docs/deal-sourcer.md`**). Google sign-in no longer overwrites non-`INVESTOR` roles. Production needs **`AUTH_SECRET`** (≥32 chars, random — use `npm run auth:secret`; **not** the same as **`AUTH_GOOGLE_SECRET`**); **`AUTH_URL`** is optional on Vercel (derived from **`VERCEL_URL`** if unset).
...
**Onboarding tracking:** `User` fields `investorProfileCompletedAt`, `ppmRiskCompletedAt`, `taxCompletedAt`, `wireInstructionsAcknowledgedAt`; sub-routes under `/dashboard/onboarding/*`; dashboard shows **outstanding count**; incomplete steps use **red borders** on the onboarding overview. **Manual bank** (`ManualBankAccount`, last-4 routing/account) on Fund page; Plaid optional legacy (not for KYC). **Tax intake:** `taxProfileJson` + PDF download via `pdf-lib` (`/api/tax-profile`, `/api/tax-profile/pdf`). **Deals:** Prisma `Deal` model (`maxSubscriptionPctOfTotalCost`, default 20) — list at `/dashboard/deals`, detail `/dashboard/deals/[slug]`. **Deal Sourcers / Employees** add deals at `/dashboard/team/deals/new` (`POST /api/deals`) or **CSV** at `/dashboard/team/deals/import` (`POST /api/deals/import-csv`); **`DealAuditLog`** records field-level changes with user + timestamp; deal **comments** with like/upvote/downvote/reply (`DealComment`, `DealCommentVote`). OG image from `propertyUrl` → `thumbnailUrl` (`src/lib/og-image.ts`). Seed migration includes Umberland (`2915-umberland-dr-atlanta`). **Subscribe:** `/dashboard/subscribe` + `/dashboard/subscribe/[slug]`; `DealSubscription` + `POST /api/deals/[slug]/subscribe`; min **$5k**, max **pct × totalCost** (`src/lib/subscription-rules.ts`). **OAuth:** set `AUTH_URL` or `APP_URL` to `https://modularequity.com` on custom domain to avoid `redirect_uri_mismatch`.
