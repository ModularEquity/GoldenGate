...
- **NextAuth (Auth.js)** — Google OAuth + credentials; JWT sessions; **`UserRole`**: `INVESTOR` vs `EMPLOYEE` (email `@modularequity.com` → Employee). Production needs **`AUTH_SECRET`** (≥32 chars, random — use `npm run auth:secret`; **not** the same as **`AUTH_GOOGLE_SECRET`**); **`AUTH_URL`** is optional on Vercel (derived from **`VERCEL_URL`** if unset).
